...
import { RagIndex, type RagSourceInput } from '../src/rag.js';
import type { EmbeddingProvider } from '../src/embeddings.js';

class FakeEmbeddingProvider implements EmbeddingProvider {
  readonly name = 'fake-hashed-bow';
...
test('treats line-ending-only changes as the same source', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'rag-mcp-server-eol-'));
  ...
  const initial = await index.ingestSources([
    { kind: 'file', locator: filePath, path: filePath, label: 'Script' }
  ]);
  assert.equal(initial.length, 1);

  const initialSources = await index.listSources();
  ...
  const updated = await index.ingestSources([
    { kind: 'file', locator: filePath, path: filePath, label: 'Script' }
  ]);
  assert.equal(updated.length, 1);

  const updatedSources = await index.listSources();
  assert.equal(updatedSources.length, 1);
  assert.equal(updatedSources[0].sourceId, initialSources[0].sourceId);
  assert.equal(updatedSources[0].chunkCount, initialSources[0].chunkCount);

  await rm(tempDir, { recursive: true, force: true });
});

test('ingests a directory recursively and skips ignored folders', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'rag-mcp-server-dir-'));
  ...
  await writeFile(path.join(docsDir, 'root.md'), '# Root\n\nDirectory ingest should find this file.', 'utf8');
  await writeFile(path.join(nestedDir, 'child.ts'), 'export const childValue = 42;', 'utf8');
  await writeFile(path.join(ignoredDir, 'skip.md'), '# Skip\n\nThis should not be ingested.', 'utf8');
  ...
  const results = await index.ingestDirectory(docsDir);
  assert.equal(results.length, 2);

  const sources = await index.listSources();
  assert.equal(sources.length, 2);

  const query = await index.search({ query: 'childValue', topK: 3 });
  assert.ok(query.length >= 1);
  assert.match(query[0].snippet, /childValue/);
