...
fn main() {
    // Prefer an explicit CODE_VERSION provided by CI; otherwise embed the
    // pinned stable CLI version used by local builds.
    let version = std::env::var("CODE_VERSION")
        .unwrap_or_else(|_| DEFAULT_CODE_VERSION.to_string());
