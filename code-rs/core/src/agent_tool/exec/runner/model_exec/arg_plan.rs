...
pub(super) fn describe_effective_execution(
    requested_model: &str,
    spec: Option<&crate::agent_defaults::AgentModelSpec>,
    prepared: &PreparedModelExecution,
) -> String {
    let effective_model = spec.map_or(requested_model, |value| value.slug);
    let source = if prepared.use_current_exe {
        "current Code executable (built-in or configured command unavailable)"
    } else {
        "configured executable"
    };
    format!(
        "requested={requested_model}; effective={effective_model}; command={}; source={source}",
...
mod tests {
    ...
    fn custom_provider_model_command_keeps_model_and_provider_arguments() {
        ...
        assert_eq!(
            describe_effective_execution("openrouter/vendor-model", None, &prepared),
            "requested=openrouter/vendor-model; effective=openrouter/vendor-model; command=coder; source=configured executable",
        );
    }
}
