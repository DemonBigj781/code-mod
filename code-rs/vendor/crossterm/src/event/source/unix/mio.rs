...
mod tests {
    ...
    #[test]
    fn terminal_eof_returns_unexpected_eof() {
        let (reader, writer) = UnixStream::pair().expect("create socket pair");
        reader
            .set_nonblocking(true)
            .expect("set reader nonblocking");
        ...
        let mut source = UnixInternalEventSource::from_file_descriptor(input)
            .expect("create terminal event source");
        drop(writer);

        let error = source
            .try_read(Some(Duration::from_secs(1)))
            .expect_err("EOF must terminate the event reader");
