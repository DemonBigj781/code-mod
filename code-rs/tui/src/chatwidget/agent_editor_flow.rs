...
mod tests {
    ...
    #[test]
    fn configure_session_op_includes_current_agent_catalog() {
        ...
        harness.with_chat(|chat| {
            let agent = AgentConfig {
                ...
                auto_drive_enabled: true,
                description: Some("Custom provider model".to_owned()),
                env: None,
                args_read_only: None,
                args_write: None,
                instructions: None,
            };
            chat.config.agents = vec![agent.clone()];

            let Op::ConfigureSession { params } = chat.current_configure_session_op() else {
                panic!("expected configure-session operation");
            };
            assert_eq!(params.agents, Some(vec![agent]));
        });
    }
}
