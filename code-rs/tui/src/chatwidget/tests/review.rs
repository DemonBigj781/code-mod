    ...
    #[test]
    fn agent_status_update_exposes_effective_execution_progress() {
    let mut harness = ChatWidgetHarness::new();
    let chat = harness.chat();
    reset_history(chat);

    let progress = "12:34:56: Effective agent execution: requested=codex-mini; effective=code-gpt-5.4-mini; command=/opt/custom-code-agent; source=configured executable";
    chat.handle_code_event(Event {
        id: "turn-effective-agent".to_string(),
        event_seq: 0,
        msg: EventMsg::AgentStatusUpdate(AgentStatusUpdateEvent {
            agents: vec![CoreAgentInfo {
                id: "agent-effective".to_string(),
                name: "Configured Agent".to_string(),
                status: "running".to_string(),
                batch_id: Some("batch-effective".to_string()),
                model: Some("code-gpt-5.4-mini".to_string()),
    ...
    let agent = chat
        .active_agents
        .iter()
        .find(|agent| agent.id == "agent-effective")
        .expect("effective agent should be visible in TUI state");
    assert_eq!(agent.model.as_deref(), Some("code-gpt-5.4-mini"));
    assert_eq!(agent.last_progress.as_deref(), Some(progress));
    }
